...
public class HoloCircleSeekBar extends View {
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		PathEffect effect=new DashPathEffect(new float[]{2,8},0);
		...
		PathEffect effect2=new DashPathEffect(new float[]{2,8},0);
		mArcColor.setPathEffect(effect2);
//		LinearGradient lg=new LinearGradient(0,0,100,100,Color.TRANSPARENT,wheel_color, Shader.TileMode.MIRROR);
//		mArcColor.setShader(lg);
	...
	protected void onDraw(Canvas canvas) {
		...
		// Draw the color wheel.灰色虚线的圆弧 270
		canvas.drawArc(mColorWheelRectangle, start_arc + 135, end_wheel
				- (start_arc), false, mColorWheelPaint);
		//绿色虚线的圆弧 270
		canvas.drawArc(mColorWheelRectangle, start_arc + 135,
	...
	private float calculateAngleFromRadians(int radians) {
		//270
		return (float) (((radians + 135) * (2 * Math.PI)) / 360);
	}
